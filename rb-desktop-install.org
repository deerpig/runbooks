...
So, let's install Firefox 50 from:
...
#+begin_src sh
cd ~/Downloads/
tar xjf firefox-50.1.0.tar.bz2
#+end_src
...
Create a symbolic link to the new Firefox:
...
Use IceWeasel, *not* Conkeror to open the followig link:
...
Conkeror without all the links being hilighted and numbered
-- something I didn't use that much anyway.

There is a lot more -- an init file that you can edit like
in Conkeror etc.  But for now I'm just happy to have back a
lot of the old functionality of Conkeror again.
