...
to do this will the openbox config files but there were too
many small hardware differences between different computers
I am running.

** Disable Caps Lock

If you aren't rebinding the caps lock key to do something
useful in emacs, it can be a major pain in the ass.  Turn it
off in ~/.config/openbox/autostart file until you find a
better use for it.
