...
I keep most of my config files in a git repo called
/.dotfiles/ and then make a symbolic link from the files in
the directory to ~/.

I keep a copy of the repo on a private VM on Kinto and the
first thing I do in the morning when I am in any office (my
week is split between my part-time gig in Phnom Penh, my
office at the College, another desktop in the Kinto-Cloud
data center and the computer in my flat at home as well as
my laptop.  They all share the same basic installation, and
I sync all of the config files and my core org-mode files
(which have all my tasks, project files, notes, journal and
blog) between all of them.  As long as I sync in the morning, and
then again before I go home the system works surprisingly
well.

I will eventually get my .emacs converted to a literate
program and weed out all of the passwords so that I can put
all of it up online for anyone to use.  But until then it
will remain private.
